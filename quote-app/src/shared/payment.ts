...
export const PAYMENT_QUEUE_SECTIONS: Array<{ key: keyof PaymentQueue; label: string; description: string }> = [
  ...
  { key: "oldDebt", label: "Nợ cũ", description: "Đơn có khoản nợ cũ riêng" },
];
...
export function shouldShowPaymentQr(paymentStatus: PaymentStatus, remainingAmount: number, oldDebtAmount = 0): boolean {
  return oldDebtAmount > 0 || (paymentStatus !== "PAID" && remainingAmount > 0);
}

export function groupPaymentQueue(quotes: QuoteRecord[]): PaymentQueue {
  const queue: PaymentQueue = { unpaid: [], deposited: [], partial: [], paid: [], oldDebt: [] };
  for (const quote of quotes) {
    if (quote.status === "CANCELLED") continue;
    if ((quote.oldDebtAmount ?? 0) > 0) {
      queue.oldDebt.push(quote);
      continue;
    }
