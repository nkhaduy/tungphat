...
(function () {
  ...
  const r2ImagePattern = /^(images|og)\/[^\s]+\.(avif|webp|png|jpe?g)$/i;

  function imageKey(value) {
    if (!value) return "";
    if (typeof value === "string") return value.trim();
    if (typeof value.get === "function") return String(value.get("key") || "").trim();
  ...
  CMS.registerEventListener({
    ...
    handler: ({ entry }) => {
      ...
      const image = imageKey(data.get("featuredImage"));
      ...
      if (image && !imagePattern.test(image) && !r2ImagePattern.test(image)) throw new Error("Ảnh đại diện phải là AVIF, WebP, PNG hoặc JPEG từ public/R2.");
