...
export default function ContactPage() {
  return (
    <>
      ...
      <SiteShell>
        <PageHero breadcrumbs={[{ label: "Trang chủ", href: "/" }, { label: "Liên hệ" }]} eyebrow="Tùng Phát tại Thủ Đức" title="Liên hệ Tùng Phát tại Thủ Đức" description={contactIntro} image={{ src: locations[0].image, alt: locations[0].imageAlt, priority: true }} actions={<><TrackedLink href={ZALO_URL} target="_blank" rel="noopener noreferrer" eventName="click_zalo" eventProperties={{ location: "contact_hero" }} className="pressable inline-flex min-h-14 items-center justify-center gap-2 bg-wood-600 px-6 text-sm font-extrabold text-white hover:bg-wood-700"><MessageCircle size={18} aria-hidden="true" />Liên hệ Zalo</TrackedLink><a href="#branch-locations" className="pressable inline-flex min-h-14 items-center justify-center gap-2 border border-forest-900/20 bg-white px-6 text-sm font-extrabold text-forest-950">Xem hai chi nhánh</a></>} />
        <ViewTracker event="view_contact_page" contentType="contact" />
        <section className="section-space bg-white">
          <div className="container-shell grid gap-10 lg:grid-cols-[.9fr_1.1fr]"><div><SectionHeader eyebrow="Thông tin liên hệ" title="Gửi mã, quy cách hoặc file cần hỏi" description="Gọi hoặc nhắn Zalo trước khi đến nếu bạn cần hỏi một mã cụ thể. Có sẵn ảnh mẫu, kích thước hay bản vẽ thì gửi kèm ngay từ đầu." /><dl className="mt-7 grid gap-4 border-t border-forest-900/10 pt-6 text-sm"><div><dt className="font-bold text-slate-600">Tên công ty</dt><dd className="mt-2 text-lg font-extrabold text-forest-950">{BUSINESS_NAME.toUpperCase()}</dd></div><div><dt className="font-bold text-slate-600">Mã số thuế</dt><dd className="mt-2 font-extrabold tabular-nums text-forest-950">{TAX_ID}</dd></div></dl></div><div className="grid gap-4 sm:grid-cols-2"><a href={PHONE_HREF} className="pressable flex min-h-32 flex-col justify-between border border-forest-900/10 bg-[#f7f8f5] p-6 hover:border-wood-500/40"><Phone size={23} className="text-wood-600" aria-hidden="true" /><span><strong className="block text-sm text-slate-600">Gọi điện</strong><span className="mt-2 block text-lg font-extrabold text-forest-950">{PHONE_DISPLAY}</span></span></a><a href={`mailto:${business.email}`} className="pressable flex min-h-32 flex-col justify-between border border-forest-900/10 bg-[#f7f8f5] p-6 hover:border-wood-500/40"><Mail size={23} className="text-wood-600" aria-hidden="true" /><span><strong className="block text-sm text-slate-600">Email</strong><span className="mt-2 block break-all font-extrabold text-forest-950">{business.email}</span></span></a></div></div>
        </section>
        <section id="branch-locations" className="section-space scroll-mt-32 bg-[#f7f8f5]"><div className="container-shell"><SectionHeader eyebrow="Hệ thống tại Tam Bình" title="Chọn chi nhánh ở Thủ Đức, TP.HCM" description="Xem ảnh mặt tiền, địa chỉ và mở Google Maps theo đúng số nhà. Nếu muốn hỏi mã hoặc quy cách trước khi ghé, hãy nhắn Zalo." /><div className="mt-9 grid gap-6 lg:grid-cols-2">{locations.map((location) => <BranchLocation key={location.id} location={location} phones={phones} />)}</div></div></section>
      </SiteShell>
    </>
  );
}
