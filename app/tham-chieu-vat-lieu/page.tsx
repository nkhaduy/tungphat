  ...
  return (
    <>
      ...
      <SiteShell>
        ...
        <section className="section-space bg-white"><div className="container-shell"><SectionHeader eyebrow="Bảng tham chiếu" title="Dữ liệu hiện có và điểm cần xác nhận" description={`Cập nhật dữ liệu: ${dataset.lastVerified}. Null không phải giá trị ước đoán; đó là dấu hiệu cần kiểm tra catalogue, mẫu hoặc mã hàng trước khi báo giá.`} /><div className="mt-8 overflow-x-auto"><table className="min-w-[920px] w-full border-collapse text-left text-sm"><caption className="sr-only">Bảng tham chiếu vật liệu</caption><thead><tr className="border-b border-forest-900/15 text-xs uppercase tracking-wide text-slate-500"><th className="p-3">Vật liệu</th><th className="p-3">Nhóm</th><th className="p-3">Ứng dụng đã công bố</th><th className="p-3">Kích thước</th><th className="p-3">Độ dày</th><th className="p-3">Bề mặt</th><th className="p-3">CNC</th></tr></thead><tbody>{dataset.materials.map((material) => <tr key={material.slug} className="border-b border-forest-900/10 align-top"><th scope="row" className="p-3 font-extrabold text-forest-950"><Link href={`/${material.slug}/`} className="underline decoration-wood-400 underline-offset-4">{material.name}</Link></th><td className="p-3 text-slate-700">{material.category}</td><td className="p-3 text-slate-700">{material.applications.join(", ")}</td><td className="p-3 text-slate-500">{displayValue(material.dimensions)}</td><td className="p-3 text-slate-500">{displayValue(material.thicknesses)}</td><td className="p-3 text-slate-500">{displayValue(material.surface)}</td><td className="p-3 text-slate-700">Kiểm tra theo file, vật liệu và số lượng</td></tr>)}</tbody></table></div></div></section>
