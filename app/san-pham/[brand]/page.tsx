...
export async function generateMetadata({ params }: BrandRouteProps): Promise<Metadata> {
  ...
  return brand
    ? createPageMetadata({ title: `${brand.name} | Sản phẩm Tùng Phát`, description: `Sản phẩm và catalogue ${brand.name} tại Tùng Phát. Gửi mã màu hoặc nhóm vật liệu để kiểm tra hàng thực tế.`, path: `/san-pham/${brand.slug}` })
    : {};
}
