  ...
  return (
    <>
      ...
      <div className="bg-[#fbfcf9]">
        <section className="bg-forest-950 py-14 text-white lg:py-20"><div className="container-shell"><nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-2 text-sm text-white/65"><Link href="/" className="min-h-11 content-center hover:text-white">Trang chủ</Link><span aria-hidden="true">/</span><Link href="/ma-mau-melamine/ba-thanh/" className="min-h-11 content-center hover:text-white">Bảng mã Melamine Ba Thanh</Link><span aria-hidden="true">/</span><span className="text-white" aria-current="page">{category.label}</span></nav><div className="mt-10 max-w-4xl"><p className="eyebrow eyebrow-on-dark">NHÓM {category.sourceLabel}</p><h1 className="mt-5 text-balance font-display text-4xl font-extrabold tracking-[-.04em] sm:text-5xl lg:text-6xl">Mã Melamine Ba Thanh {category.label.toLowerCase()}</h1><p className="mt-5 max-w-3xl text-pretty text-base leading-8 text-white/72">{category.intro}</p></div></div></section>
