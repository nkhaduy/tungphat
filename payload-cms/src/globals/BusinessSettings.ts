...
export const BusinessSettings: GlobalConfig = {
  ...
  admin: { group: 'Cấu hình website', description: 'Thông tin liên hệ, địa điểm và CTA dùng trên website.' },
  ...
  fields: [
    ...
    { name: 'website', label: 'Website canonical', type: 'text', required: true, defaultValue: 'https://mdftungphat.com', validate: (value: unknown) => value === 'https://mdftungphat.com' || 'Bắt buộc https://mdftungphat.com' },
    ...
    stringArray('serviceAreas', 'Khu vực phục vụ', { required: true, minRows: 1 }),
    {
      ...
      fields: [
        vietnameseText('locationId', 'ID', { required: true }), vietnameseText('shortId', 'Mã ngắn', { required: true }),
        ...
        { name: 'embedSrc', label: 'URL nhúng Google Maps', type: 'text', required: true }, { name: 'directionsUrl', label: 'URL chỉ đường', type: 'text', required: true },
      ],
    },
