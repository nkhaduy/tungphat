...
function chronological<T extends z.ZodRawShape>(schema: z.ZodObject<T>) {
  return schema.superRefine((value, context) => {
    ...
    if (
      dated.publishedAt &&
      dated.updatedAt &&
      dated.updatedAt < dated.publishedAt
    ) {
      context.addIssue({
        code: "custom",
        path: ["updatedAt"],
...
export const articleSchema = chronological(
  z.object({
...
export const productSchema = chronological(
  z.object({
...
export const projectSchema = chronological(
  z.object({
...
export const servicePageSchema = chronological(
  z.object({
    title: z.string().min(8).max(120),
    slug,
...
export const staticPagesSettingsSchema = z.object({
  ...
  contactIntro: z.string().min(40).max(300),
        ...
        catalogues: z
          .array(
            z.object({
              ...
              pdfUrl: z
                .union([z.literal(""), z.string().regex(/^\/[^\s]+\.pdf$/i)])
                .default(""),
            }),
          )
          .default([]),
