...
export function auditStaticOutput({ rootDir, siteUrl, expectedDirectAnswerRoutes = [] }: AuditOptions) {
  ...
  const pages = files.map((file) => {
    ...
    const internalLinks = [...html.matchAll(/<a\b[^>]*href=(?:"([^"]*)"|'([^']*)')/giu)].map((match) => match[1] ?? match[2]).filter((href) => href && href.startsWith("/") && !href.startsWith("//") && !href.startsWith("/api/") && !/\.(?:csv|json|png|jpe?g|webp|svg|pdf|xml|txt)$/iu.test(href));
