  ...
  return (
    <form ref={formRef} onSubmit={onSubmit} className="rounded-xl border border-forest-900/10 bg-white p-5 shadow-sm sm:p-7" noValidate={false}>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        {type === "quote" && <><label className="text-sm font-bold text-forest-950">Sản phẩm / mã hàng<input className={inputClass} name="product" maxLength={160} placeholder="Nếu đã xác định" /></label><label className="text-sm font-bold text-forest-950">Độ dày<input className={inputClass} name="thickness" maxLength={80} placeholder="Nếu đã xác định" /></label><label className="text-sm font-bold text-forest-950">Kích thước<input className={inputClass} name="dimensions" maxLength={160} placeholder="Dài × rộng hoặc danh sách cắt" /></label><label className="text-sm font-bold text-forest-950">Số lượng<input className={inputClass} name="quantity" maxLength={100} placeholder="Số tấm hoặc số chi tiết" /></label></>}
      </div>
