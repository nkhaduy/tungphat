    ...
    <article id={location.id} aria-labelledby={`${location.id}-title`} className="scroll-mt-32 overflow-hidden border border-forest-900/10 bg-white shadow-card">
      ...
      <div className="p-6 sm:p-7">
        ...
        <div className="mt-6 grid gap-2 sm:grid-cols-2">
          <Link href={branchPathForLocationId(location.id)} className="pressable inline-flex min-h-12 items-center justify-center gap-2 border border-forest-900/20 px-5 text-center text-sm font-extrabold text-forest-950 hover:border-forest-900">Xem chi tiết chi nhánh</Link>
          <TrackedLink href={location.directionsUrl} target="_blank" rel="noopener noreferrer" eventName="click_directions" eventProperties={{ location: location.shortId }} className="pressable inline-flex min-h-12 items-center justify-center gap-2 bg-forest-900 px-5 text-sm font-extrabold text-white hover:bg-forest-800">Mở Google Maps <ExternalLink size={17} aria-hidden="true" /></TrackedLink>
        </div>
      </div>
    </article>
  );
