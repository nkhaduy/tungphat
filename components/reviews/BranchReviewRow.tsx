...
export function BranchReviewRow({ branch, direction }: { branch: ReviewBranch; direction: "left" | "right" }) {
  ...
  if (!reviews.length) return <div data-review-branch={branch.branchKey} className="grid min-h-[287px] gap-5 md:grid-cols-[minmax(13rem,16rem)_minmax(0,1fr)]"><RatingSummary branch={branch} /><div className="grid place-items-center rounded-[0.9rem] border border-dashed border-forest-900/15 bg-white/60 p-6 text-center text-sm text-slate-500">{branch.count === 0 ? "Chưa có đánh giá." : "Google hiện chưa cung cấp nội dung đánh giá cho chi nhánh này. Bạn có thể xem hồ sơ Google để biết thêm."}</div></div>;
