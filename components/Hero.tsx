...
/* eslint-disable @next/next/no-img-element -- Native picture avoids cross-breakpoint preloads in a static export. */
...
export function Hero() {
  ...
  return (
    <section
      id="trang-chu"
      data-homepage-hero
      data-hero-desktop-sources={slides.map(({ desktop }) => desktop).join(",")}
      data-hero-mobile-sources={slides.map(({ mobile }) => mobile).join(",")}
      className="relative min-h-[620px] overflow-hidden bg-forest-950 pt-[72px] text-white lg:min-h-[100svh] lg:pt-0"
    >
      <AnimatePresence initial={false}>
        ...
        >
          ...
          <img
            ...
            width={1916}
            height={821}
            loading={active === initialActive ? "eager" : "lazy"}
      ...
      <div className="hero-content container-shell relative flex min-h-[548px] flex-col justify-center py-14 lg:min-h-[100svh]">
        ...
        >
          <p className="hero-eyebrow mb-5 flex items-center gap-3 text-xs font-bold uppercase tracking-[.16em] text-orange-300">
            <span className="h-0.5 w-10 bg-wood-500" /> {t.heroCompany}
          </p>
          <h1 className="hero-headline font-display font-extrabold tracking-[-.03em]">
            <span className="block">
              {t.heroTitle1}{" "}
              <strong className="font-extrabold text-wood-500">
                {t.heroTitle2}
              </strong>
            </span>
            <span className="block">
          ...
          <div className="hero-cta mt-7 flex flex-col gap-3 sm:flex-row">
            <TrackedLink
              href="#san-pham"
              eventName="view_product_category"
              eventProperties={{ location: "hero" }}
              className="inline-flex min-h-[52px] items-center justify-center gap-2 bg-wood-700 px-6 text-[15px] font-bold transition hover:bg-wood-800"
            >
            ...
            <TrackedLink
              ...
              eventProperties={{ location: "hero" }}
              aria-label={t.heroCtaQuote}
              className="inline-flex min-h-[52px] items-center justify-center gap-2 border border-white/55 bg-white/[0.12] px-6 text-[15px] font-bold backdrop-blur-sm transition hover:bg-white hover:text-forest-950"
            >
              <MessageCircle size={17} /> {t.heroCtaQuote}
            </TrackedLink>
          </div>
        </motion.div>
      </div>
