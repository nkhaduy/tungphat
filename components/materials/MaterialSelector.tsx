  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[.8fr_1.2fr]">
      <form onSubmit={handleSubmit} className="grid gap-5 border border-forest-900/10 bg-[#f7f8f5] p-6 sm:p-8" aria-labelledby="selector-title">
        <div>
          <p className="eyebrow">Bộ lọc định hướng</p>
          <h2 id="selector-title" className="mt-3 text-2xl font-extrabold text-forest-950">Chọn vật liệu từ yêu cầu ban đầu</h2>
          <p className="mt-3 text-sm leading-7 text-slate-700">Kết quả là gợi ý có điều kiện từ dữ liệu đã xác minh, không thay thế mẫu, bản vẽ hoặc bước xác nhận mã hàng.</p>
        </div>
      ...
      <section aria-live="polite" aria-labelledby="selector-result-title" className="border border-forest-900/10 bg-white p-6 sm:p-8">
        <p className="eyebrow">Kết quả có điều kiện</p>
        <h2 id="selector-result-title" className="mt-3 text-2xl font-extrabold text-forest-950">{result ? "Nhóm nên kiểm tra trước" : "Bắt đầu bằng bốn thông tin"}</h2>
        {result ? <div className="mt-6 grid gap-4">{result.map((material, index) => <article key={material.slug} className="border border-forest-900/10 p-5"><div className="flex items-start justify-between gap-4"><h3 className="text-lg font-extrabold text-forest-950">{index + 1}. {material.name}</h3><span className="text-xs font-bold text-wood-600">Điểm định hướng {material.score}</span></div><p className="mt-2 text-sm leading-7 text-slate-700">{material.reasons.join(" ") || "Cần bắt đầu từ thông tin hạng mục và quy cách."}</p><p className="mt-3 text-xs leading-6 text-slate-500"><strong>Lưu ý:</strong> {material.caveats.join(" ")}</p></article>)}</div> : <ul className="mt-6 grid gap-3 text-sm leading-7 text-slate-700"><li>Hạng mục: tủ, mặt bàn, kệ hoặc chi tiết dạng tấm.</li><li>Môi trường: khô, có độ ẩm hoặc có nước trực tiếp.</li><li>Bề mặt: vân gỗ tự nhiên, bề mặt phủ hoặc chưa chốt.</li><li>Gia công: có cần cắt, khoan, soi rãnh hay CNC không.</li></ul>}
      </section>
    </div>
  );
}
