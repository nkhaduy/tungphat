        ...
        <div className="mx-auto max-w-4xl rounded-2xl border border-forest-900/10 bg-white p-6 shadow-[0_18px_50px_rgba(7,59,40,.08)] sm:p-9 lg:p-11">
          ...
          {step === 3 ? (
            <form className="mt-8" onSubmit={handleSubmit} aria-labelledby="finder-step-three">
              ...
              <label className="mt-4 block text-sm font-bold text-forest-950">Nội dung yêu cầu<textarea name="message" aria-label="Nội dung yêu cầu" value={message} onChange={(event) => setMessage(event.target.value)} className="mt-2 min-h-28 w-full rounded-md border border-forest-900/15 bg-white px-4 py-3 text-sm font-normal" placeholder="Ví dụ: cần soi rãnh theo bản vẽ và kiểm tra mã bề mặt…" autoComplete="off" /></label>
              <p className="mt-4 flex items-start gap-2 text-xs leading-5 text-slate-600"><ClipboardCheck size={16} className="mt-0.5 shrink-0 text-forest-800" aria-hidden="true" />Sau khi mở Zalo, gửi kèm file kỹ thuật hoặc bản phác thảo để Tùng Phát đối chiếu yêu cầu.</p>
