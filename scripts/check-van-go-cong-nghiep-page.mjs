...
const routePath = "/van-go-cong-nghiep/";
const canonicalUrl = "https://mdftungphat.com/van-go-cong-nghiep/";
const baselineWords = 427;
const requiredLinks = ["/van-mdf/", "/mdf-chong-am/", "/go-ghep/"];
const requiredHeadingPatterns = [
  /Ván gỗ công nghiệp là gì\?/iu,
  /Những nhóm vật liệu nào đang được giới thiệu/iu,
  /Cách chọn nhóm vật liệu theo nhu cầu/iu,
  /Các tiêu chí cần xác nhận trước khi mua/iu,
  /MDF, MDF chống ẩm và gỗ ghép khác nhau/iu,
  /Vai trò của bề mặt hoàn thiện/iu,
  /hỗ trợ gia công CNC/iu,
  /Checklist kiểm tra hàng và báo giá/iu,
];
const forbiddenClaims = [
  /luôn có sẵn/iu,
  /đầy đủ mọi chủng loại/iu,
  /đại lý chính thức/iu,
  /giá (?:rẻ|thấp) nhất/iu,
  /(?:số|top)\s*1/iu,
  /tốt nhất/iu,
...
const nonexistentMaterialRoute = /^\/(?:mfc|plywood|melamine|laminate|acrylic|veneer|hdf|osb)(?:\/|$)/iu;
...
const expectedCtas = [
  { href: "https://zalo.me/0909259160", event: "click_zalo", location: "van-go-cong-nghiep_hero", target: "_blank" },
  { href: "tel:+84909259160", event: "click_phone", location: "van-go-cong-nghiep_hero", target: "" },
  { href: "https://zalo.me/0909259160", event: "click_zalo", location: "van-go-cong-nghiep_specs", target: "_blank" },
  { href: "https://zalo.me/0909259160", event: "click_zalo", location: "van-go-cong-nghiep_specs", target: "_blank" },
];

const runtimeValue = process.env.VAN_GO_CONG_NGHIEP_CHECK_ORIGIN ?? process.argv[2];
if (!runtimeValue) {
  console.error("Thiếu VAN_GO_CONG_NGHIEP_CHECK_ORIGIN hoặc origin ở đối số đầu tiên.");
  process.exit(1);
}
...
const screenshotDirectory = process.env.VAN_GO_CONG_NGHIEP_SCREENSHOT_DIR
  ? path.resolve(process.env.VAN_GO_CONG_NGHIEP_SCREENSHOT_DIR)
  : null;
const auditOnly = process.env.VAN_GO_CONG_NGHIEP_AUDIT_ONLY === "1";
const errors = [];
...
const { direct, pageUrl, results } = await auditProductPage({
  ...
  routePath,
  screenshotDirectory,
  screenshotPrefix: "van-go-cong-nghiep",
});
...
check(desktop.title === "Ván gỗ công nghiệp: cách chọn theo nhu cầu | Tùng Phát", `Title không đúng intent hoặc lặp brand: ${desktop.title}.`);
check(
  desktop.description === "Hướng dẫn hiểu ván gỗ công nghiệp, phân biệt MDF, MDF chống ẩm và gỗ ghép, chọn theo nhu cầu, chuẩn bị quy cách, CNC và thông tin kiểm tra báo giá.",
...
check(desktop.mainWords >= 1000, `Main content cần tối thiểu 1.000 từ; nhận ${desktop.mainWords}.`);
check(desktop.mainWords <= 1700, `Main content vượt 1.700 từ; nhận ${desktop.mainWords}.`);
for (const pattern of placeholderPatterns) check(!pattern.test(desktop.mainText), `Phát hiện placeholder: ${pattern}.`);
for (const pattern of forbiddenClaims) check(!pattern.test(desktop.mainText), `Phát hiện forbidden claim: ${pattern}.`);
check(!/\b\d[\d.,]*\s*(?:đ|vnđ|vnd)\b/iu.test(desktop.mainText), "Phát hiện giá tiền trong main content.");
check(
  !/(?:mọi|tất cả)\s+(?:loại\s+)?gỗ ghép[^.]{0,80}(?:là|thuộc)\s+ván (?:gỗ )?công nghiệp/iu.test(desktop.mainText),
  "Phát hiện khẳng định taxonomy tuyệt đối về gỗ ghép.",
);
...
for (const pathname of uniqueInternalPaths) {
  ...
  check(!nonexistentMaterialRoute.test(pathname), `Không được link tới route vật liệu chưa tồn tại: ${pathname}.`);
...
check(!schemaTypes.includes("Product"), "Category guide không được xuất Product schema.");
...
if (errors.length > 0) {
  console.error(`Ván gỗ công nghiệp page validation failed (${errors.length}):`);
  for (const error of errors) console.error(`- ${error}`);
  process.exit(1);
}

console.log("Ván gỗ công nghiệp page validation pass.");
