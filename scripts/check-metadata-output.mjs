...
for (const route of [...legalRoutes, ...placeholderRoutes]) {
  ...
  const { canonicals, robots, openGraphUrls } = metadataTags(readFileSync(outputPath, "utf8"));
  if (canonicals.length !== 1 || canonicals[0]?.href !== expectedUrl) {
    errors.push(`${route}: canonical phải xuất hiện đúng một lần và bằng ${expectedUrl}`);
  }
  if (openGraphUrls.length !== 1 || openGraphUrls[0]?.content !== expectedUrl) {
    errors.push(`${route}: og:url phải xuất hiện đúng một lần và bằng ${expectedUrl}`);
  }

  const robotsContent = robots[0]?.content.toLowerCase() ?? "";
  const expectedDirective = placeholderRoutes.includes(route) ? "noindex" : "index";
  if (
    robots.length !== 1 ||
...
if (!existsSync(sitemapPath)) {
  ...
  for (const route of legalRoutes) {
    if (!sitemapUrls.includes(`${siteUrl}${route}`)) {
      errors.push(`${route}: URL indexable phải có trong sitemap với trailing slash`);
    }
  }
  for (const route of placeholderRoutes) {
    const normalizedRoute = route.replace(/\/$/, "");
    if (sitemapUrls.some((url) => url.replace(/\/$/, "") === `${siteUrl}${normalizedRoute}`)) {
...
if (errors.length) {
  console.error(`Metadata output validation thất bại (${errors.length} lỗi):\n- ${errors.join("\n- ")}`);
  process.exit(1);
}

console.log("Metadata output validation pass: 9 URL có canonical, og:url, robots và sitemap đúng.");
