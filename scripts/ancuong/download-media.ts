...
export type MediaDownloadStatus =
  "downloaded" | "duplicate" | "missing" | "invalid" | "failed" | "dry-run";
...
export interface MediaManifest {
  ...
  summary: Record<MediaDownloadStatus, number> & {
    total: number;
    totalBytes: number;
  };
}
...
export function inspectImageBytes(
  ...
  const detected =
    pngInfo(bytes) ?? gifInfo(bytes) ?? jpegInfo(bytes) ?? webpInfo(bytes);
  if (!detected)
    throw new Error(
      `Unsupported or corrupt image payload${declaredMime ? ` (${declaredMime})` : ""}`,
    );
...
function mediaIdentity(
  record: Pick<
    MediaDownloadInput,
    "sourceUrl" | "productSourceId" | "productCode" | "role"
  >,
): string {
...
export async function downloadMedia(
  ...
  const manifestPath =
    options.manifestPath ?? path.join(options.outputDir, "media-manifest.json");
  const previous = options.force
    ? undefined
    : await readJsonIfExists<MediaManifest>(manifestPath);
  const previousByIdentity = new Map(
    previous?.records.map((record) => [mediaIdentity(record), record]),
  );
  const previousBySourceUrl = new Map<string, MediaManifestRecord>();
  ...
  for (const record of previous?.records ?? []) {
    if (
      record.sha256 &&
      record.localPath &&
      (await exists(path.join(options.outputDir, record.localPath)))
    ) {
      checksumPaths.set(record.sha256, record.localPath);
      if (!previousBySourceUrl.has(record.sourceUrl)) {
        previousBySourceUrl.set(record.sourceUrl, record);
      }
    }
  }
  ...
  const worker = async () => {
    while (cursor < orderedInputs.length) {
      ...
      if (
        cached?.localPath &&
        cached.sha256 &&
        (await exists(path.join(options.outputDir, cached.localPath)))
      ) {
        records[index] = { ...cached, ...input };
        continue;
      }
      const reusable = previousBySourceUrl.get(input.sourceUrl);
      if (reusable?.localPath && reusable.sha256) {
        records[index] = {
          ...reusable,
          ...input,
          status: "duplicate",
          duplicateOf: reusable.localPath,
        };
        continue;
      }
      if (options.dryRun) {
      ...
      try {
        ...
        if (existingWrite) {
          ...
          const writePromise = atomicWriteBinary(
            path.join(options.outputDir, localPath),
            bytes,
          ).then(() => localPath);
        ...
        records[index] = {
          ...input,
          status: /challenge|image payload|dimensions/i.test(message)
            ? "invalid"
            : "failed",
          error: message,
          originalFilename: originalFilename(input.sourceUrl),
  ...
  const concurrency = Math.max(
    1,
    Math.min(options.concurrency ?? 3, 8, orderedInputs.length || 1),
  );
...
export async function run(options: CliOptions): Promise<MediaManifest> {
  const normalized = await readJsonIfExists<unknown>(
    path.join(paths.normalized, "catalogue.json"),
  );
  const products = Array.isArray(normalized)
    ? normalized
    : normalized &&
        typeof normalized === "object" &&
        Array.isArray((normalized as { records?: unknown[] }).records)
      ? (normalized as { records: unknown[] }).records
      : [];
  ...
  for (const value of products) {
    ...
    if (product.primaryImage?.sourceUrl)
      inputs.push({
  ...
  return downloadMedia(inputs, {
    outputDir: paths.media,
    manifestPath: path.join(paths.normalized, "media-manifest.json"),
