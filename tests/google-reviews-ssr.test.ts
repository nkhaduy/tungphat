...
import { getFeaturedReviews, getGoogleReviewUrls, TrustindexReviews, type TrustindexReviewData } from "../components/reviews/TrustindexReviews";
...
describe("Trustindex reviews SSR", () => {
  ...
  it("deduplicates the featured set and excludes one-star cards without changing the aggregate", () => {
    const data: TrustindexReviewData = {
      sourceUrl: "https://public.trustindex.io/reviews/mdftungphat.com/lang/vi",
      source: "Google",
      rating: 4.8,
      ...
      reviews: [
        ...
        { id: "short", reviewerName: "Năm sao", avatarUrl: null, rating: 5, text: "Tốt", date: "2026.01.02" },
        { id: "useful", reviewerName: "Năm sao", avatarUrl: null, rating: 5, text: "Tư vấn rõ ràng, giao hàng đúng hẹn và vật liệu đúng mô tả.", date: "2026.01.03" },
      ],
    };
    const html = renderToStaticMarkup(createElement(TrustindexReviews, { data }));
    expect(html).toContain("Năm sao");
    ...
    expect(getFeaturedReviews(data.reviews)).toEqual([data.reviews[2]]);
  });
