...
describe("static SEO output audit", () => {
  it("counts indexability and catches duplicate metadata and canonical errors", () => {
    ...
    writeFileSync(path.join(rootDir, "other", "index.html"), html("Same title", "https://example.com.evil/other/").replace("</body>", '<a href="/missing/">Missing</a><script type="application/ld+json">{broken}</script></body>'));
