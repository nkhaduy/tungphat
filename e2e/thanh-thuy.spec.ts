...
test("brand catalogue supports code search without creating hotlinks", async ({
  page,
}) => {
  await page.goto("/thuong-hieu/thanh-thuy/");
  await expect(
    page.getByRole("heading", { level: 1, name: /Catalogue Thanh Thuỳ/ }),
  ).toBeVisible();
...
test("sparse code page is useful but noindex", async ({ page }) => {
  ...
  await expect(
    page.getByRole("heading", { level: 1, name: /142 Roman Oak/ }),
  ).toBeVisible();
  ...
  const zalo = page.getByRole("link", {
    name: "Gửi mã 142 của Thanh Thuỳ qua Zalo",
  });
...
test("ready product has factual Product schema without Offer", async ({
  page,
}) => {
  ...
  expect(product).toMatchObject({
    sku: "LP 101/104G",
    brand: { name: "Thanh Thuỳ" },
    material: "Laminate",
  });
  expect(product).not.toHaveProperty("offers");
});

test("mobile catalogue has labels, focus and no horizontal overflow", async ({
  page,
}) => {
  await page.setViewportSize({ width: 390, height: 844 });
  await page.goto("/thuong-hieu/thanh-thuy/");
  await page
    .getByRole("searchbox", { name: "Tìm tên hoặc mã Thanh Thuỳ" })
    .focus();
  await expect(
    page.getByRole("searchbox", { name: "Tìm tên hoặc mã Thanh Thuỳ" }),
  ).toBeFocused();
  await page
